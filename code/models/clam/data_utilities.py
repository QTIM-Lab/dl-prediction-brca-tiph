...
class TCGABRCA_MIL_Dataset(Dataset):
	...
	def split_dataset(self):
		...
		for fold in range(self.n_folds):
			assert len(self.dataset_dict['case_id']) == len(train_dict[fold]['case_id']) + len(
				val_dict[fold]['case_id']) + len(test_dict[fold]['case_id'])
	...
	# Method: __getitem__ with tile-level augmentation
	def __getitem__(self, idx):
		...
		# --- Data augmentation per tile ---
		# Use the fixed list of augmentation files
		folder = os.path.dirname(features_h5)
		aug_files = ["hed_rs0.h5", "hed_rs1.h5", "hed_rs2.h5", "hed_rs3.h5"]
		n_tiles = features.shape[0]
		mask = np.random.rand(n_tiles) < 0.2  # 20% chance per tile
		if mask.sum() > 0:
			aug_indices = np.where(mask)[0]
			chosen_files = np.random.choice(aug_files, size=len(aug_indices))
			for aug_file in np.unique(chosen_files):
				indices = aug_indices[chosen_files == aug_file]
				aug_path = os.path.join(folder, aug_file)
				with h5py.File(aug_path, "r") as f_aug:
					features_aug = f_aug["features"][()]
				features[indices] = features_aug[indices]
		# -----------------------------------------
		features = torch.from_numpy(features)

		# Get ssGSEA score and binarize it based on the threshold
...
class TCGABRCA_MIL_DatasetRegression(TCGABRCA_MIL_Dataset):
	...
	def __getitem__(self, idx):
		...
		# --- Data augmentation (same strategy) ---
		folder = os.path.dirname(features_h5)
		aug_files = ["hed_rs0.h5", "hed_rs1.h5", "hed_rs2.h5", "hed_rs3.h5"]
		n_tiles = features.shape[0]
		mask = np.random.rand(n_tiles) < 0.2
		if mask.sum() > 0:
			aug_indices = np.where(mask)[0]
			chosen_files = np.random.choice(aug_files, size=len(aug_indices))
			for aug_file in np.unique(chosen_files):
...
class TCGABRCA_MIL_DatasetClinicalSubtype(Dataset):
	...
	def load_tcga_brca_labels(self):
		tcga_brca_labels = pd.read_excel(
			os.path.join(self.base_data_path, 'Annotations', 'breast_cancer_tcga_labels_1.xlsx'))
		tcga_brca_labels = tcga_brca_labels.copy()[
			['CLID', 'er_status_by_ihc', 'pr_status_by_ihc', 'HER2.newly.derived']]
		...
		for case_id in self.svs_fpaths_dict.keys():
			if case_id in wsi_genex_label_dict and case_id in features_h5_dict:
				for svs_path in self.svs_fpaths_dict[case_id]:
					...
					for ssgea in ssgea_scores_list:
						ssgea_ext = self.get_case_id(wsi_path_or_name=ssgea, mode='extended')
						if case_id_ext == ssgea_ext and feature_h5_fname in features_h5_dict[
							case_id] and case_id in self.tcga_brca_labels_dict.keys():
							dataset_dict['case_id'].append(case_id)
							dataset_dict['svs_fpath'].append(svs_path)
							dataset_dict['features_h5'].append(feature_h5_fname)
							dataset_dict['ssgea_id'].append(ssgea)
							...
							dataset_dict['c_subtype_label'].append(
								self.tcga_brca_labels_dict[case_id]['c_subtype_label'])
		assert len(dataset_dict['case_id']) == len(dataset_dict['svs_fpath'])
		assert len(dataset_dict['case_id']) == len(dataset_dict['features_h5'])
		assert len(dataset_dict['case_id']) == len(dataset_dict['ssgea_id'])
		assert len(dataset_dict['case_id']) == len(dataset_dict['ssgsea_scores'])
