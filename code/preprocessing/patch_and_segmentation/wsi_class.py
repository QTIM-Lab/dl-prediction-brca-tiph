...
# Project Imports
from wsi_utilities_methods import savePatchIter_bag_hdf5, initialize_hdf5_bag, save_hdf5, screen_coords, isBlackPatch, isWhitePatch, to_percentiles
from wsi_utilities_classes import isInContourV1, isInContourV2, isInContourV3_Easy, isInContourV3_Hard, Contour_Checking_fn
...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def _getPatchGenerator(self, cont, cont_idx, patch_level, save_path, patch_size=256, step_size=256, custom_downsample=1,
        ...
        if custom_downsample > 1:
            ...
            if self.verbose:
                print("Custom Downsample: {}, Patching at {} x {}, But Final Patch Size is {} x {}".format(custom_downsample, patch_size, patch_size, target_patch_size, target_patch_size))
    ...
    # Method: Draw and visualize heatmaps
    ...
    # Method: Load HistoQC Segmentation Masks
    def initHistoQCSegmentation(self, mask_file_path):
        """
        Initializes the histogram-based quality control segmentation by categorizing contours
        in the mask image as tissue contours or hole contours.

        Args:
            mask_file_path (str): Path to the mask image file.

        Returns:
            contours_tissue (list): List of tissue contours.
            holes_tissue (list of list of ndarrays): Each sublist corresponds to the holes of the tissue contour at the same index.
        """

        # Load the mask image
        mask_image = cv2.imread(mask_file_path, cv2.IMREAD_GRAYSCALE)

        # Binarize the mask image (optional, depending on the storage format)
        _, binary_mask = cv2.threshold(mask_image, 0, 255, cv2.THRESH_BINARY)

        # Find all contours in the binary mask image
        all_contours, _ = cv2.findContours(binary_mask, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)

        # Find external contours in the binary mask image
        external_contours, hierarchy = cv2.findContours(binary_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)


        # Create lists to store the tissue contours and hole contours
        contours_tissue = []
        holes_tissue = []
        ...
        # Iterate over each external contour
        for ext_contour in external_contours:
            holes = []

            # Check if the external contour contains any smaller contours (holes)
            for i, contour in enumerate(all_contours):
                if np.array_equal(contour, ext_contour):
                    continue  # Skip comparing the contour with itself

                if cv2.pointPolygonTest(ext_contour, (int(contour[0][0][0]), int(contour[0][0][1])), False) > 0:
                    holes.append(contour)

            # Convert external contour coordinates to int32
            ext_contour = ext_contour.astype(np.int32)

            contours_tissue.append(ext_contour)
            holes_tissue.append(holes)

        # Get dimensions of the WSI
        wsi_dimensions = self.wsi.dimensions

        # Get dimensions of the mask image
        mask_dimensions = mask_image.shape[::-1]
        # cv2 image shape is in (height, width), so reverse it to (width, height)

        # Calculate scale ratios
        scale_x = wsi_dimensions[0] / mask_dimensions[0]
        scale_y = wsi_dimensions[1] / mask_dimensions[1]

        # Scale the contours
        contours_tissue = [contour * np.array([scale_x, scale_y], dtype=np.int32) for contour in contours_tissue]
        holes_tissue = [[hole * np.array([scale_x, scale_y]).astype(np.int32) for hole in holes] for holes in
                        holes_tissue]
