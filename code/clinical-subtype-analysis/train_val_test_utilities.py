...
def test_pipeline(test_set, config_json, device, checkpoint_dir, fold):
    ...
    for _, input_data_dict in enumerate(test_loader):
        ...
        features, ssgsea_scores, ssgsea_scores_bin, c_subtype, case_id, svs_path, = input_data_dict["features"].to(device), input_data_dict["ssgsea_scores"].to(device), input_data_dict["ssgsea_scores_bin"].to(device), input_data_dict["c_subtype"].to(device), input_data_dict["case_id"].to(device), input_data_dict["svs_path"].to(device)
        ...
        case_ids.extend(list(case_ids.cpu().detach().numpy()))
        svs_paths.extend(list(svs_path.cpu().detach().numpy()))
